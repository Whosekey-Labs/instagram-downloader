...
export async function collectAll({read,scroll,atBottom,blocked,wait=waitFor,signal,expected,onProgress,maxSteps=1800}) {
  const posts=new Map();let idle=0;
  for(let step=0;step<maxSteps;step++) {
    signal.throwIfAborted();
    if(blocked())throw new Error('Instagram 로그인 또는 접근 확인 화면이 나타나 전체 수집을 중단했습니다.');
    const before=posts.size;
    for(const post of read())posts.set(post.code,post);
